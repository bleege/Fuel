...
class OverviewViewController: UIViewController, OverviewContractView, MKMapViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func refreshMap() {
        ...
        fuelStopAnnotations.append(contentsOf: fuelStops.map({(value: FuelStop) in
            ...
            let annPoint = MKMapPoint.init(pin.coordinate)
            let pointRect = MKMapRect.init(x: annPoint.x, y: annPoint.y, width: 0.0, height: 0.0)
            if (mapRect.isNull) {
                mapRect = pointRect
            } else {
        ...
        if (!mapRect.isNull) {
            let padding = UIEdgeInsets(top: 75, left: 75, bottom: 75, right: 75)
            mapView.setVisibleMapRect(mapRect, edgePadding: padding, animated: true)
        }
    }
    
    func zoomToUserLocation() {
        mapView.region = MKCoordinateRegion.init(center: mapView.userLocation.coordinate, latitudinalMeters: 1000, longitudinalMeters: 1000)
    }
    
    func displayStopOnMap(index: Int) {
        let ann = fuelStopAnnotations[index]
        mapView.region = MKCoordinateRegion.init(center: ann.coordinate, latitudinalMeters: 1000, longitudinalMeters: 1000)
